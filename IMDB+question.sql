...
-- Most of movies released in 2017
...
-- Highest number of movies released in March Month
...
-- USA seems to have produced more movies than India in 2019
...
-- Overall Drama genre are highest 
...
-- Total 3289 movies belong to one genre
...
-- Action movies seems to have highest average duration time
...
-- avg_rating range is between 1 and 10
-- total vote range is between 100 and 725138
-- Median rating range is between 1 and 10

-- which concludes there are no outliers in the table
...
-- RSVP should partner with Dream Warrior Pictures or National Theatre Live 
...
INNER JOIN ratings AS r
ON m.id = r.movie_id
WHERE median_rating = 8 AND date_published BETWEEN '2018-04-01' 
AND '2019-04-01'
GROUP BY median_rating;

-- 361 movies are having median rating of 8
...
-- height column has more null values
...
SELECT production_company, SUM(total_votes) AS vote_cnt,
